...
router.post('/', (req, res) => {
  try {
    const { client_id, total, items, date, status, notes, user_id } = req.body;
    ...
    try {
      // Calcular data de vencimento (30 dias a partir de hoje por padrão)
      ...
      if (hasSaleId) {
        ...
        const result = accountStmt.run(
          ...
          description,
          total,
          dueDate.toISOString().split('T')[0], // Formato YYYY-MM-DD
          'Pendente',
          saleId
        );
        console.log(`✅ Conta a receber criada com ID: ${result.lastInsertRowid}`);
      } else {
