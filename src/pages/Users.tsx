...
const Users = () => {
  ...
  const toggleActiveMutation = useMutation({
    mutationFn: (id: number) => {
      ...
      return apiService.toggleUserActive(id, user.id);
    },
    onSuccess: (data) => {
      queryClient.invalidateQueries({ queryKey: ['users'] });
      toast({
  ...
  const changePasswordMutation = useMutation({
    mutationFn: ({ userId, currentPassword, newPassword }: { userId: number; currentPassword: string; newPassword: string }) =>
  ...
  const adminResetPasswordMutation = useMutation({
    mutationFn: ({ targetUserId, newPassword }: { targetUserId: number; newPassword: string }) => {
      ...
      return apiService.adminResetPassword(user.id, targetUserId, newPassword);
    },
    onSuccess: () => {
      toast({
        title: "Senha alterada!",
