...
class ApiService {
  ...
  async createProduct(data: { name: string; price: number; stock: number; unit?: string }) {
    return this.request<any>('/products', {
      method: 'POST',
  ...
  async updateProduct(id: number, data: { name?: string; price?: number; stock?: number; unit?: string }) {
    return this.request<any>(`/products/${id}`, {
      method: 'PUT',
      body: JSON.stringify(data),
    });
  ...
  async addProductStock(id: number, quantity: number, notes?: string) {
  ...
  async createSale(data: { client_id?: number; total: number; items: Array<{ product_id: number; quantity: number; price: number }> }) {
    return this.request<any>('/sales', {
      method: 'POST',
      body: JSON.stringify(data),
    });
  ...
  async toggleUserActive(id: number, userId: number) {
  ...
  async adminResetPassword(adminUserId: number, targetUserId: number, newPassword: string) {
  ...
  async exportDatabase(userId: number): Promise<Blob> {
    const response = await fetch(`${API_BASE_URL}/database/export?userId=${userId}`, {
      method: 'GET',
    });
  ...
  async importDatabase(file: File, userId: number): Promise<{ success: boolean; message: string; backupPath?: string }> {
  ...
  async getDatabaseInfo(userId: number) {
    return this.request<{
      path: string;
      size: number;
